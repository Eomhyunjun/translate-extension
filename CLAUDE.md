...
- **`src/shared.js`** — small, pure, dependency-free helpers shared by every context (`clamp`, `isPlainObject`, `isSupportedPageUrl`, `isTransientExtensionError`, `parseAllowedOpenAICompatibleEndpoint`, `isAllowedOpenAICompatibleEndpoint`). Loaded right after `defaults.js` everywhere (same wiring: `importScripts` in background, manifest `content_scripts` + the `executeScript` file lists, and `<script>` tags in popup/options HTML). It assigns the helpers as bare globals on `globalThis`, so the four realms don't each keep their own copy. **When a helper is needed in more than one context, put it here instead of duplicating it — and keep all the load-order sites in sync.**
